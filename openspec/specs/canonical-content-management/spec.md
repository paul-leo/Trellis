# canonical-content-management Specification

## Purpose
Command-line CRUD (`trellis skill`/`trellis mcp` `list`/`add`/`remove`)
for authoring canonical skills and MCP servers — an alternative to
hand-editing `~/.trellis/skills/<name>/SKILL.md` or
`~/.trellis/mcp/servers.yaml` directly. Canonical-side only; never
touches any agent's native config (that's `sync`/`mcp sync`'s job).
Created by archiving change trellis-canonical-cli-crud.
