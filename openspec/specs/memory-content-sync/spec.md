# memory-content-sync Specification

## Purpose
`trellis memory sync` — ingests canonical `memories/*.md` content into
`@modelcontextprotocol/server-memory`'s own on-disk JSON-lines knowledge-
graph file, distinct from `memory-defaults` (which only covers
distributing the server's *config* to agents, not its content). Created
by archiving change trellis-memory-sync.
