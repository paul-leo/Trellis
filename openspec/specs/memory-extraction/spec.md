...
## Purpose
`trellis memory extract` — the reverse of `memory sync`: reads the
shared `server-memory` MCP server's own JSON-lines graph file and writes
its real, non-`trellis-memory` entities out as new canonical
`memories/*.md` files. Closes `memory-content-sync`'s own explicitly-
named "per-agent extraction into canonical remains a separate, open
gap." Created by archiving change trellis-memory-extraction.
