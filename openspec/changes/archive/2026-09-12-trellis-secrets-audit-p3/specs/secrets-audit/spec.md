...
findings list (literal-value + unexpected-name, across every present
agent) is non-empty, and SHALL exit zero when it is empty. The audit
SHALL NOT modify any file it reads.

#### Scenario: Any finding at all yields a non-zero exit
- **WHEN** at least one finding of either kind exists across any present
  agent
- **THEN** `trellis secrets audit` exits with code 1

#### Scenario: No findings yields a zero exit
- **WHEN** every present agent's real config file passes both checks
...
- **WHEN** `trellis secrets audit` runs against any agent's real config
  file, finding or no finding
- **THEN** that file's bytes on disk are unchanged after the run
